...
class ReservationController extends AbstractController
{
    ...
    public function acceptReservation(Reservation $reservation, Security $security, EntityManagerInterface $entityManager, MailerInterface $mailer): Response
{
    $user = $security->getUser();
    ...
    // Tworzenie powiadomienia dla klienta
    $notification = new Notification();
    $notification->setUser($reservation->getUser());
    $notification->setMessage("✅ Właściciel potwierdził Twoją rezerwację dla: " . 
        $reservation->getCar()->getBrand() . " " . $reservation->getCar()->getModel() . 
        " od " . $reservation->getStartDate()->format('Y-m-d') . " do " . 
    ...
    $email = (new Email())
        ...
        ->text("✅ Właściciel potwierdził Twoją rezerwację dla: " . 
            $reservation->getCar()->getBrand() . " " . $reservation->getCar()->getModel() . 
            " od " . $reservation->getStartDate()->format('Y-m-d') . " do " . 
            $reservation->getEndDate()->format('Y-m-d') . ".");
    ...
    $this->addFlash('success', 'Rezerwacja została zaakceptowana, a użytkownik został powiadomiony.');
    return $this->redirectToRoute('app_reservations');
}
...
public function confirmReservation(Request $request, Car $car, EntityManagerInterface $entityManager, MailerInterface $mailer): Response
{
    ...
    // Dodanie powiadomienia dla właściciela samochodu
    $notification = new Notification();
    $notification->setUser($car->getOwner());
    $notification->setMessage("🚗 Nowa rezerwacja: {$car->getBrand()} {$car->getModel()} od {$startDate->format('Y-m-d')} do {$endDate->format('Y-m-d')}.");
    $entityManager->persist($notification);
    ...
    $emailOwner = (new Email())
        ->from('noreply@twojastrona.pl')
        ->to($car->getOwner()->getEmail())
        ->subject('🚗 Nowa rezerwacja Twojego samochodu!')
        ->text("Masz nową rezerwację dla swojego samochodu: {$car->getBrand()} {$car->getModel()}.\nData: {$startDate->format('Y-m-d')} - {$endDate->format('Y-m-d')}.");

    $mailer->send($emailOwner);

    // **Powiadomienie e-mail dla najemcy**
    $emailRenter = (new Email())
        ->from('noreply@twojastrona.pl')
        ->to($this->getUser()->getEmail())
        ->subject('✅ Potwierdzenie rezerwacji')
        ->text("Twoja rezerwacja samochodu: {$car->getBrand()} {$car->getModel()} została zatwierdzona.\nData: {$startDate->format('Y-m-d')} - {$endDate->format('Y-m-d')}.");

    $mailer->send($emailRenter);
